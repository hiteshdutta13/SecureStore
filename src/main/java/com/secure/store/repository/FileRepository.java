...
public interface FileRepository extends JpaRepository<File, Long> {
    ...
    List<File> findBy(@Param("userId") Long userId, @Param("status") Status status);

    @Query("SELECT d FROM File d WHERE d.user.id = :userId AND d.folder.id = :folderId AND d.status = :status")
    List<File> findBy(@Param("userId") Long userId, @Param("folderId") Long folderId, @Param("status") Status status);
