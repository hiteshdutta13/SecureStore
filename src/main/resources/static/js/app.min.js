...
const HTMLStyle = new function() {
    ...
    this.contextOfFile = function(file, shared) {
        ...
        dataAttrs.push({attr: "type", value: HTMLStyle.fileType(file.name)});
...
const File = new function() {
    ...
    this.share = function(el) {
        ...
        const modalContainer = $("#fileShareModal");
        const modalTitle = modalContainer.find(".modal-title");
        const modalBody = modalContainer.find(".modal-body");
        modalTitle.empty().append("<i class='fa "+(File.fileIcons[type] || File.fileIcons["default"])+" text-success'></i> ").append(filename);
        modalBody.empty();
        ...
        modalBody.append(divInput);
        const divSelectedUsers = $("<div/>").attr({class: "mb-3 mt-3 selected-users"});
        modalBody.append(divSelectedUsers);
        modalBody.find(".search-users").on("blur", function(e) {
            setTimeout(function() {modalBody.find(".user-results").empty();}, 1000);
        });
        const modalSearchUsers = modalContainer.find(".search-users");
        modalSearchUsers.on("keyup", function(e) {
            var query = $(this).val();
            var callBack = function(response) {
                modalBody.find(".user-results").empty();
                if(response.data) {
                    const modalUserResults = modalBody.find(".user-results");
                    ...
                    const modalSelectedUsers = modalBody.find(".selected-users");
        ...
        modalContainer.find(".btnShareFile").on("click", function(e) {
            e.preventDefault();
            var toUsers = [];
            modalBody.find(".selected-users").find("span").each(function(index) {
