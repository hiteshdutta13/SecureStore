...
const File = new function() {
    ...
    this.show = function(id, fileName, type) {
        var myModal = new bootstrap.Modal(document.getElementById('fileViewModal'), {
            keyboard: false
        });
        $("#fileViewModal").find(".modal-title").empty().append(fileName);
        $("#fileViewModal").find(".modal-body").empty();
        $("#fileViewModal").find(".modal-body").addClass("text-center");
        if(type === "image") {
            const image = $("<img/>").attr({src: File.context+"/api/file/image/"+id, class: "img-fluid"});
            ...
            const url = File.context+"/api/file/pdf/"+id;
            ...
            async function showDoc() {
                ...
                const response = await fetch(File.context+"/api/file/other/"+id);
                if (!response.ok) {
...
const Drive = new function() {
    this.events = function() {
        ...
        $(".open-file").on("click", function(e) {
            e.preventDefault();
            File.show($(this).attr("id"), $(this).attr("name"), $(this).data("type"));
        });
        ...
        if(response.data) {
            ...
            if(sharedFilesWithU.length > 0) {
                ...
                for(let index = 0; index < sharedFilesWithU.length; index++) {
                    divSharedFileWithURow.append(HTMLStyle.createFile(sharedFilesWithU[index], true));
                }
                $(".driveContent").append("<h6 class='text-muted mt-4'>Shared files with you</h6>");
            ...
            if(sharedFilesByU.length > 0) {
                ...
                for(let index = 0; index < sharedFilesByU.length; index++) {
                    divSharedFileByURow.append(HTMLStyle.createFile(sharedFilesByU[index], true));
                }
                $(".driveContent").append("<h6 class='text-muted mt-4'>Shared files by you</h6>");
