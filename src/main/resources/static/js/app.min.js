...
const Folder = new function() {
    ...
    this.properties = function(id, folderName) {
        ...
        $("#fileViewModal").find(".header-custom").remove();
...
const File = new function() {
    ...
    this.upload = function(folderId) {
        var files = $('#fileSelected')[0].files;
        if(files === undefined || files.length === 0) {
            $("#fileSelected").addClass("is-invalid");
            $("#fileSelected").closest(".invalid-feedback").append("Please choose a file/Or files or valid file format. Allowed types: JPG, PNG, PDF, DOC, DOCX, TXT, XLS, XLSX.");
            return false;
        }
        // Allowed file types
        const allowedTypes = [
    ...
    this.show = function(el) {
        ...
        $("#fileViewModal").find(".modal-title").empty().append("<i class='fa "+(File.fileIcons[type] || File.fileIcons["default"])+" text-success'></i> ").append(fileName);
        $("#fileViewModal").find(".modal-body").empty();
        const div = $("<div/>").attr({class: "col-sm-12 border-bottom border-success pe-2 header-custom"});
        const aShare = $("<a/>").attr({class: "float-end btn btn-sm share-view-file", href:"javascript:;", id: id, title: "Share", name: fileName}).append('<i class="fa-solid fa-share-nodes text-success"></i>');
        aShare.attr("data-type", type);
        const aDownload = $("<a/>").attr({class: "float-end btn btn-sm download-view-file", href:"javascript:;", id: id, title: "Download", name: fileName}).append('<i class="fa-solid fa-download text-success"></i>');
        div.append(aDownload);
        if(!shared) {
            div.append(aShare);
        }
        $("#fileViewModal").find(".header-custom").remove();
        $("#fileViewModal").find(".modal-body").before(div);
    ...
    this.renameContext = function(disabled) {
        if(disabled) {
            $(".file-context").find(".rename-file").addClass("disabled");
        }else {
            $(".file-context").find(".rename-file").removeClass("disabled");
        }
    };
    this.rename = function(id, fileName) {
        const input = $("<input/>").attr({type: "text", class:"col-sm-10 col-md-10 col-lg-10 col-xl-10 col-xxl-10", value: fileName})
        const close = $("<a/>").attr({class:"col-sm-1 col-md-1 col-lg-1 col-xl-1 col-xxl-1", href:"javascript:;", title: 'Cancel'}).append('<i class="fa-solid fa-xmark text-danger"></i>');
        $(".edit-file-"+id).replaceWith(input);
        input.after(close);
    ...
    this.share = function(el) {
        ...
        $("#fileShareModal").find(".modal-title").empty().append("<i class='fa "+(File.fileIcons[type] || File.fileIcons["default"])+" text-success'></i> ").append(filename);
...
const HTMLStyle = new function() {
    this.contextOfFolder = function(folder) {
        const ul = $("<ul/>").attr({class: "dropdown-menu w-auto position-absolute"});
    ...
    this.contextOfFile = function(file, shared) {
        ...
        if(!shared) {
            ...
            aShare.attr("data-type", HTMLStyle.fileType(file.name));
            const liShare = $("<li/>").append(aShare);
            ul.append(liShare);

            const aRename = $("<a/>").attr({class: "dropdown-item rename-file", href: "javascript:;", id: file.id, name: file.originalName}).append("Rename");
            const liRename = $("<li/>").append(aRename);
            ul.append(liRename);
    ...
    this.listViewDetailRow = function(data, isFile) {
        const divM = $("<div/>").attr({class: "col-sm-4 col-md-4 col-lg-4 col-xl-4 col-xxl-4"});
        if(isFile) {
            const divR1 = $("<div/>").attr({class: "row"});
            const divR1C1 = $("<div/>").attr({class: "col-sm-6 col-md-6 col-lg-6 col-xl-6 col-xxl-6 text-end fs-s text-muted"});
            divR1C1.append("Date modified:");
            divR1.append(divR1C1);
            const divR1C2 = $("<div/>").attr({class: "col-sm-6 col-md-6 col-lg-6 col-xl-6 col-xxl-6 fs-s text-muted"});
            divR1C2.append(data.createdDateTime);
            divR1.append(divR1C2);
            divM.append(divR1);

            const divR2 = $("<div/>").attr({class: "row"});
            const divR2C1 = $("<div/>").attr({class: "col-sm-6 col-md-6 col-lg-6 col-xl-6 col-xxl-6 text-end fs-s text-muted"});
            divR2C1.append("Size:");
            divR2.append(divR2C1);
            const divR2C2 = $("<div/>").attr({class: "col-sm-6 col-md-6 col-lg-6 col-xl-6 col-xxl-6 fs-s text-muted"});
    ...
    this.createFolder = function(folder, view) {
        ...
        const divCol = $("<div/>").attr({class: rowClasses+" py-2 border border-1 rounded-1 folder-box"});
        ...
        const divColName = $("<a/>").attr({class: nameClasses+" folder-name text-decoration-none text-dark text-ellipsis", href: "javascript:;", id: folder.id, title: "Right-click for more actions."});
    ...
    this.createFile = function(file, view, shared) {
        var rowClasses = "col-sm-5 col-md-5 col-lg-5 col-xl-5 col-xxl-5";
        ...
        divColR.append(divColIcon);
        const divColName = $("<a/>").attr({class: nameClasses+" file-name open-file text-decoration-none text-dark text-ellipsis", href: "javascript:;", id: file.id, name: file.originalName, title: "Right-click for more actions."});
    ...
    this.open = function(response) {
        ...
        if(response.data) {
            ...
            for(let index = 0; index < files.length; index++) {
                divFileRow.append(HTMLStyle.createFile(files[index], response.data.view, false));
            }
            if(files.length > 0) {
                if(folders.length > 0) {
        ...
        if(response.data) {
            ...
            if(sharedFilesWithU.length > 0) {
                ...
                for(let index = 0; index < sharedFilesWithU.length; index++) {
                    divSharedFileWithURow.append(HTMLStyle.createFile(sharedFilesWithU[index].file, response.data.view, true));
                }
            ...
            if(sharedFilesByU.length > 0) {
                ...
                for(let index = 0; index < sharedFilesByU.length; index++) {
                    divSharedFileByURow.append(HTMLStyle.createFile(sharedFilesByU[index].file, response.data.view, true));
                }
...
const Bin = new function() {
    this.contextOfFile = function(file) {
        ...
        const liRestore = $("<li/>").append(aDelete);
        ul.append(liRestore);
    ...
    this.initialize = function() {
        ...
        $(".restore-all").on("click", function(e){
            ...
            $(".bin-check").each(function(i){
                $(this).prop("checked", false);
                ids.push($(this).attr("id").split("checkFile_")[0]);
            });
            console.log(ids);
            let callBackConfirm = function() {
                let callBack = function(response) {
        ...
        $(".delete-all").on("click", function(e){
            ...
            Message.confirm("Yes, Delete Permanently!", "This will permanently delete the selected item(s). Are you sure? This action cannot be undone.", callBackConfirm);
        });
    };
};
